...
As a concrete example, consider Scala's bounded type polymorphism. In the following snippet, type inference must instantiate \lstinline!U! to \lstinline!Even!, which succeeds because a refinement type is a subtype of its base type, and therefore \lstinline!Even <: Int! satisfies the bound \lstinline!U <: T!. The result type is then inferred as \lstinline!Even!. This example was used to motivate \emph{abstract refinements}~\cite{vazou2013}. In our system, it follows directly from subtyping:
