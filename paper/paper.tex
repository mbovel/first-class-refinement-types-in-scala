...
\begin{itemize}
  \item We present the \textbf{design} of first-class refinement types in Scala~3: their syntax, semantics, and interaction with Scala's existing type system, including subtyping, type inference, bounded polymorphism, overloading, and implicits (\S\ref{sec:design}). A distinctive design choice is a \textbf{partial-correctness semantics that requires no termination checking}: predicates are ordinary, possibly diverging pure terms whose meaning is defined operationally rather than by translation into a classical logic---a refinement holds if its predicate evaluates to $\texttt{true}$ whenever it terminates (\S\ref{sec:termination}).
