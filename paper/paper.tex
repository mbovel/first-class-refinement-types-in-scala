...
We present the design of first-class refinement types for Scala~3, where refinements are ordinary types that participate in subtyping, inference, and pattern matching alongside existing language features. We prove type soundness of a core, pure calculus mechanized in Rocq, combining dependent function types, bounded polymorphism, positive equi-recursive types, union and intersection types, and refinement types under a partial-correctness semantics using a fuel-bounded definitional interpreter and semantic typing. A distinctive design choice is that predicates themselves may diverge: type soundness requires no termination assumptions, and we prove that a diverging predicate is vacuously true and can never be used to prove false. Finally, we implement our design as a prototype extension of the Scala~3 compiler with a lightweight e-graph-based solver for predicate entailment.
\end{abstract}
...
Integrating refinement types into the host language's type system is not entirely new: dependently typed languages such as Dafny~\cite{leino2010} and F*~\cite{swamy2016} already treat refinement types as first-class citizens of their type checkers, and use subtyping to relate a refinement type to its base type. However, these languages were designed from the ground up around verification and do not feature the rich subtyping, bounded polymorphism, overloading, and implicit resolution that characterize object-oriented languages like Scala. To our knowledge, this is the first integration of refinement types into a language where subtyping is a central, pervasive feature, with classes, traits, mixin composition, path-dependent types, and bounded polymorphism all relying on it. The challenge we address is showing that refinement types can be added to such a feature-rich, pre-existing type system while preserving its existing behavior.
...
\begin{itemize}
  \item We present the \textbf{design} of first-class refinement types in Scala~3: their syntax, semantics, and interaction with Scala's existing type system, including subtyping, type inference, bounded polymorphism, overloading, and implicits (\S\ref{sec:design}). A distinctive design choice is a \textbf{partial-correctness semantics for predicates}: predicates are ordinary, possibly diverging pure terms whose meaning is defined operationally rather than by translation into a classical logic---a refinement holds if its predicate evaluates to $\texttt{true}$ whenever it terminates. Type soundness requires no termination assumptions; termination is needed only to justify some logical entailment rules (\S\ref{sec:termination}).
  \item We prove \textbf{type soundness} of a core, pure calculus capturing the essential features of our design, mechanized in Rocq~\cite{rocq} (\S\ref{sec:formalization}). The operational semantics is given by a fuel-bounded definitional interpreter, and soundness is established via semantic typing, where types are interpreted as predicates on values. Soundness requires no termination assumptions; we further prove that allowing diverging predicates is not degenerate: such a predicate is indistinguishable from $\texttt{true}$ and can never be used to prove $\texttt{false}$ (Lemmas~\ref{lem:and-true}--\ref{lem:diverge-vacuous}). To our knowledge, this is the first mechanized soundness proof that combines refinement types with union and intersection types, bounded polymorphism (with lower and upper bounds), and positive equi-recursive types, a combination needed to model Scala's type system but not covered by prior mechanizations~\cite{hamza2019, borkowski2022, sun2024}.
...
A concrete path toward enforcing purity is offered by \emph{capture checking}~\cite{xu2025}, a new, experimental extension of the Scala~3 compiler.\footnote{The extensions are described in the Scala~3 documentation: \url{https://nightly.scala-lang.org/docs/reference/experimental/capture-checking/}.} It consists of several layers. At its core, capture checking tracks \emph{capabilities}: references that confer the authority to perform effects, such as file-system or network access. The types of values and methods record which capabilities they capture or use; a \emph{pure} function is one that captures nothing. An extension named \emph{Stateful Capabilities} refines this model to mutable state by distinguishing read and write permissions: mutable state must live in classes marked \lstinline!Stateful!, methods that update it must be marked \lstinline!update!, and reads of such state are tracked as well, as uses of a read-only capability. A further layer, \emph{separation checking}~\cite{xu2026}, requires every mutable field to reside in a \lstinline!Stateful! class and enforces exclusive, un-aliased access to written state, so that no mutation goes untracked. Finally, \emph{safe mode}~\cite{odersky2026} enables all these checks and closes the remaining escape hatches---unchecked casts, reflection, unsafe library access---so that capabilities cannot be silently discarded.

With these layers in place, purity checking of predicates reduces to a simple condition: reject any predicate that references a capability, calls a method that uses one, or reads a mutable variable. Weaker checks are also conceivable: tracking the \emph{read set} of each predicate (the read-only capabilities it uses) and invalidating established refinement facts whenever an element of that set is written to. Separation checking would make such invalidation sound, as all writes are explicit and un-aliased. Similar ideas underpin refinement type systems for Rust, where ownership likewise guarantees exclusive access to written state (\S\ref{sec:related-work}). We leave these integrations to future work.
...
In this paper we give the implication judgment a semantic definition only; developing a syntactic proof system for predicate entailment is future work. Our solver (\S\ref{sec:solver}) implements a set of entailment rules whose termination-sensitive instances are currently unverified: like purity (\S\ref{sec:purity}), termination of the functions called in predicates is assumed rather than checked, and a termination checker is left as future work. Note that within the checked predicate fragment (\S\ref{sec:purity}), every construct except function calls trivially terminates, and termination only matters for specific rules, such as the simplification of $t \mathbin{\texttt{\&\&}} \texttt{false}$ to $\texttt{false}$ above, or the use of a predicate from a called function's result type. Crucially, even with a termination checker in place, proofs would be required only for the specific functions those rules touch---never systematically for every function used in a predicate. We believe this is essential for adoption: Scala is a general-purpose language, and systematically requiring termination proofs would create a significant barrier.
...
The type $\mu X.\, A$ denotes an equi-recursive type, interpreted as the intersection of all step-indexed approximations (see \S\ref{sec:interpretation}). Unlike iso-recursive systems, no explicit fold/unfold terms are needed; instead, the equivalence $\mu X.\, A \cong A[X \bind \mu X.\, A]$ is established via subtyping rules (\S\ref{sec:subtyping}).
...
The rules for successful evaluation $\rho \vdash a \Downarrow v$ are given in Figure~\ref{fig:eval}. Closures capture the environment at the point of creation: evaluating $\lambda x{:}A.\, b$ under~$\rho$ yields $\langle \rho, \lambda x.\, b \rangle$. Application evaluates the function and argument, then evaluates the body in the closure's environment extended with the argument.
...
\paragraph{Notation.} As in the rest of the paper, we state the remaining lemmas with named variables (\S\ref{sec:language}), writing $\mathrm{FV}(A)$ for the free (term and type) variables of~$A$. The mechanization states them in de~Bruijn form, where extending the environment corresponds to an explicit index shift on the type and the freshness side conditions below are implicit.

\paragraph{Weakening and substitution for the interpretation.}
The semantic typing and subtyping proofs require that the value interpretation is well-behaved under changes to the environment. The key lemmas establish that the interpretation depends only on the variables the type mentions: extending the environment with a fresh binding leaves it unchanged, and a variable can be substituted away when the environment determines its value.
...
\begin{itemize}
  ...
  \item \emph{Argument hoisting.} When a method signature contains qualified types that refer to other parameters, the corresponding arguments are lifted to local \lstinline!val! bindings~(\S\ref{sec:anf}). This is needed so that run-time checks can refer to the argument without re-evaluating it.
...
In the formalization (\S\ref{sec:formalization}), \textsc{T-App} (Figure~\ref{fig:typing}) requires the argument to be a variable~$y$, so that the return type $B[x \bind y]$ is a simple substitution of one variable for another. In practice, Scala allows arbitrary expressions as arguments, so the implementation must recover this structure on demand through two mechanisms.
...
How to apply a dependent method \lstinline!f: (x: A) => B! to an arbitrary expression~\lstinline!e!? We cannot blindly substitute, because the return type $B[x \bind e]$ may not be defined if \lstinline!e! is outside of the predicate language fragment. This is a well-known problem in refinement type systems. Borkowski et al.~\cite{borkowski2022} solve it with term-level existential types, giving \lstinline!f(e)! the type $\exists x.\, A \land B[x]$ (following Knowles and Flanagan~\cite{flanagan2006}); Hamza et al.~\cite{hamza2019} use a ``let-in-type'' construct $\mathsf{let}\; x = e\;\mathsf{in}\; B$.
...
We evaluate our implementation along two axes: compilation time overhead and expressiveness compared to two existing systems for Scala---Schmid and Kun\v{c}ak's previous prototype~\cite{schmid2016} and the Stainless verifier~\cite{hamza2019}.

\paragraph{Compilation overhead.}
We measure single-shot compilation time on a suite of small benchmark programs, comparing a baseline (compilation without refinement checking) against the full system. Each benchmark is compiled in isolation, and we report the median over 10 iterations after 30 warm-up iterations (Note to reviewers: will be increased for the final version). Where possible, we port the same program to Schmid's system and to Stainless to compare overhead across implementations. Figure~\ref{fig:bench-table} summarizes the results.

On our benchmarks, our system (labelled ``First-class'' in the figure) adds modest overhead for typical programs. The outlier is \texttt{maximumBig}, an artificial stress test that creates 100 singleton-type elements and checks that each is even; as expected, the cost grows with the number of refinement checks. Schmid's system and Stainless both show higher overhead on the benchmarks they support, which we attribute to their use of external SMT solvers.
...
The current overhead is acceptable for a prototype but leaves room for improvement. The solver is invoked hundreds of times per file, often with repeated queries, because Scala's subtype checker may re-check the same pair of types multiple times during inference. Caching solver results would eliminate most of this redundancy. The solver itself is also unoptimized: it is a textbook e-graph implementation with no indexing or incremental updates.

\paragraph{Impact on code that does not use the feature.}
Because refinement checking is integrated into the compiler rather than layered on top, a natural concern is whether it affects existing code. To assess this, we run the full Dotty continuous-integration suite with the feature enabled. It passes unchanged: this includes compiling the Scala compiler itself (${\sim}200\,000$ lines of code), its ${\sim}10\,000$ compilation tests (${\sim}300\,000$ lines), and a community build of 50 open-source projects (${\sim}700\,000$ lines), indicating that the extension introduces no backward incompatibility. On the official Scala compiler benchmark suite,\footnote{\url{https://lampepfl.github.io/scala3-benchmarks/}} enabling the feature shows no noticeable regression on either the 11 ``Big'' benchmarks, which compile real-world libraries totalling ${\sim}70\,000$ lines, or the 20 ``Small'' benchmarks. In short, when the feature is unused, it adds no measurable compilation cost and changes no behavior.

\input{bench_table}

%\input{bench_overhead_pct}

\paragraph{Expressiveness.}
The three systems differ in which programs they can express. First-class refinement types support refinements on class types (not just base types) and can capture arbitrary values from the enclosing scope in predicates. Schmid's implementation is limited to refinements on \lstinline!Int! types; it cannot express refinements over class-typed values (e.g., \lstinline!{v: Matrix if v.width > 0}!) because its SMT encoding treats class types as opaque. It also cannot propagate refinements through generic code, since its qualifier inference operates in a separate phase from Scala's type checker. Stainless supports richer specifications through \lstinline!require! and \lstinline!ensuring! clauses with a full SMT backend, but refinements are not types: they do not compose with bounded polymorphism, type inference, or pattern matching. Our system integrates with all of these by construction, since refinement types are ordinary Scala types.
...
We reviewed the history of refinement types in the introduction. Flanagan's hybrid type checking~\cite{flanagan2006} introduced the idea of automatically inserting run-time checks when the static system cannot discharge a refinement obligation, requiring a careful formalization of the interaction between static and dynamic checking. In our system, run-time checks are not inserted automatically: the programmer explicitly requests them via \lstinline!.runtimeChecked! or pattern matching (\S\ref{sec:design}), which the type system treats as an ordinary conditional branch. This makes the formalization straightforward---no special rules are needed for the static/dynamic boundary. Moreover, our run-time checks are restricted to first-order predicates, which avoids the blame assignment problem that arises when monitoring higher-order contracts~\cite{keil2015}: since functions cannot be checked eagerly, systems that automatically insert run-time checks for higher-order values must carefully track which party is at fault when a contract violation is detected at a later call site.
%
Compared to our system, Liquid Haskell~\cite{vazou2014, vazou2017} requires termination for soundness, while we prove partial correctness, avoiding the need for termination checks.
%
In the systems programming space, refinement types have been combined with mutation by leveraging ownership---the avenue we sketch for Scala in \S\ref{sec:purity}. Flux~\cite{lehmann2023} attaches pure refinement values to mutable locations in Rust: exclusive ownership permits \emph{strong updates}, changing a location's refinement at each write, while a mutable borrow permits only \emph{weak updates} that preserve the borrowed type as an invariant---unless the borrow is declared a \emph{strong reference}, whose updated refinement is propagated to the owner through an \lstinline!ensures! clause in the function signature. Thrust~\cite{thrust2025} and RefinedRust~\cite{gaher2024} instead type a mutable borrow as a pair of its current value and a \emph{prophecy variable} (a \emph{borrow name} in RefinedRust) standing for the borrow's final value, fixed when the borrow is released. RefinedC~\cite{sammler2021} and RefinedRust combine refinement types with ownership types for C and Rust respectively, with foundational soundness proofs via the Iris separation logic framework, and Verus~\cite{lattuada2023} provides SMT-based verification for Rust using linear ghost types; these systems target memory safety and low-level invariants, complementing our focus on a high-level language with subtyping, polymorphism, and equi-recursive types. For TypeScript, Refined TypeScript~\cite{vekris2016} allows refinements to mention object state only through immutable fields---the discipline that the purity condition of \S\ref{sec:purity} enforces for all state---and recovers flow-sensitive reasoning for local variables via an SSA translation.
...
AI tools were used in the preparation of this paper and its artifacts, primarily Claude Opus 4.5/4.6:

\begin{itemize}
  \item \textbf{Writing.} Content was drafted by the authors in outline form and then refined with AI assistance, paragraph by paragraph. Technical content is the authors'---only wording and phrasing were AI-assisted. AI also assisted with typesetting the typing rules.
  \item \textbf{Proofs.} Theorem and lemma statements were written by the authors, while many proof bodies were generated with AI assistance.
  \item \textbf{Code.} The core implementation predates widespread agentic AI tools, but recent bug fixes and refactorings were AI-assisted. AI-assisted commits are explicitly marked in the git history (\texttt{Co-authored-by: Claude Opus 4.5/4.6}), which will be available in the final artifact. Graph generation scripts and benchmarking code were heavily AI-assisted.
\end{itemize}

GitHub Copilot was also used for code autocompletion, and Google and ChatGPT for general research questions.
