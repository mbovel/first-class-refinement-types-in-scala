...
%% Allow a little extra interword stretch as a last resort, so long
%% inline-code fragments do not protrude into the margin.
...
\acmArticle{409}
\acmMonth{10}
\acmSubmissionID{oopslab26main-p1986-p}
\received{2026-03-23}
...
\keywords{Refinement Types, Scala, Definitional Interpreter, Semantic Types, Rocq}
...
The consequences of this separation run deeper than usability. Because the refinement checker operates after or alongside the host type checker, it must duplicate much of the host's type inference machinery, an engineering burden that also limits expressiveness. Schmid and Kun\v{c}ak~\cite{schmid2016} implemented qualified types for an earlier version of the Scala compiler (Dotty) with a refinement checker ``largely independent of Scala's own type checker.'' This architecture meant refinement types could not interact with Scala's type inference (they could not be inferred as type arguments in generic code) and required a separate qualifier inference algorithm that proved difficult to scale.

Userland Scala libraries such as Iron~\cite{iron} and Refined~\cite{refined} take a different trade-off: they encode refinement-like checks using opaque types and implicit evidence, benefiting from Scala's own type inference and tooling, but at the cost of expressiveness; refinement proofs are limited to what Scala's implicit resolution can discharge, ruling out dedicated efficient decision procedures for equality, arithmetic, ordering, or other domain-specific theories.
...
\end{lstlisting}
%
Here, \lstinline!Int with x % 2 == 0! is a regular Scala type: it participates in subtyping, generics, pattern matching, type bounds, overloading, and implicits, eliminating the two-mental-models problem. There is one type system, one set of error messages, and one inference algorithm.
...
As a concrete example, consider Scala's bounded type polymorphism. In the snippet below, the ordering is defined for \lstinline!Int! only, yet \lstinline!maximum! applied to even numbers should return \lstinline!Even!; this challenge motivated \emph{abstract refinements}~\cite{vazou2013}. In our system, bounded polymorphism and subtyping suffice: \lstinline!List(2, 4, 6)! is typed as \lstinline!List[2 | 4 | 6]!, type inference instantiates \lstinline!T! to \lstinline!Int!, whose \lstinline!Ordering! instance performs the comparisons, and \lstinline!U! to \lstinline!2 | 4 | 6!, and the result checks against \lstinline!Even! since each literal satisfies the predicate:
...
\end{lstlisting}
\vspace{-1em}
\caption{A refinement type interacting with bounded type polymorphism.}
\label{fig:example1}
\end{figure}

As another example, consider overload resolution, a feature characteristic of object-oriented languages. Below, we define two overloads of \lstinline!min!: one for sorted lists and one for arbitrary lists. Because refinement types participate in overload resolution, the compiler dispatches \lstinline!min(l)! to the more specific overload when \lstinline!l! is known to be sorted:
...
We measure single-shot compilation time on a suite of small benchmark programs, comparing each system against its own baseline: the same programs without refinement annotations, compiled with checking disabled. Where possible, we port the same program to Schmid's system and to Stainless. Baselines are per-system because the systems build on different compiler versions and the ported programs differ slightly in code shape. Benchmarks run under JMH; each benchmark is compiled with 150 single-shot warm-up iterations followed by 20 measurement iterations, and the whole experiment is repeated in 10 independent runs. Each run is reduced to the mean of its measurement iterations; Table~\ref{tab:bench-table} reports the mean of the per-run means with 95\% confidence intervals (Student's $t$), and the overhead~$\Delta$ as the difference between checked and baseline times.

On our benchmarks, our system (labelled ``First-class'' in the table) adds modest overhead: 0--12\% of the baseline compilation time. This is consistently less than the two other systems: on the benchmarks they support, Schmid's system adds 20--38\% and Stainless at least 56\%.
...
Our mechanization builds on Amin and Rompf's~\cite{amin2017} technique of proving type safety via a fuel-bounded definitional interpreter. One of their key insights is that big-step semantics sidesteps the ``bad bounds'' problem that plagues small-step proofs for languages with type members. However, their development proves syntactic type safety (progress and preservation) rather than defining semantic types.
%
Owens et~al.~\cite{owens2016} introduced functional big-step semantics, and Paraskevopoulou et~al.~\cite{paraskevopoulou2021} use a similar interpreter-based approach with step-indexed logical relations for multi-pass compiler verification. In both systems, fuel is consumed at every evaluation step, whereas ours bounds evaluation depth (\S\ref{para:fuel}).
%
Semantic typing, interpreting types as predicates or relations on values, has a long history in logical relations~\cite{timany2024}. Giarrusso et~al.~\cite{giarrusso2020} develop a semantic model for DOT, with subtyping and recursive types using step-indexed logical relations and a ``later'' modality in the type syntax. We avoid step-indexing entirely by requiring strict positivity for recursive type binders (following Hamza et~al.~\cite{hamza2019}), enabling a direct fixpoint construction via intersected approximations (\S\ref{sec:soundness}).

\paragraph{Refinement types in other languages.}
We reviewed the history of refinement types in the introduction. Flanagan's hybrid type checking~\cite{flanagan2006} introduced the idea of automatically inserting run-time checks when the static system cannot discharge a refinement obligation. In our system, run-time checks are not inserted automatically: the programmer explicitly requests them via \lstinline!.runtimeChecked! or pattern matching (\S\ref{sec:design}), which the type system treats as an ordinary conditional branch, so no special rules are needed for the static/dynamic boundary. Moreover, our run-time checks are restricted to first-order predicates, which avoids the blame assignment problem that arises when monitoring higher-order contracts~\cite{keil2015}: since functions cannot be checked eagerly, a violation detected at a later call site must be attributed to the party at fault.

Liquid Haskell~\cite{vazou2014, vazou2017} brings refinement types to Haskell as a separate plugin, with refinement inference running apart from GHC's own type checking, whereas our refinements are typed and inferred by the compiler itself. Termination also plays a different role: Liquid Haskell by default requires every function to be proven terminating, while we need termination only in the solver, to justify certain normalization and unfolding rules (\S\ref{sec:termination}). Finally, Liquid Haskell discharges obligations with an SMT solver over decidable theories, while our solver (\S\ref{sec:solver}) combines e-graphs with normalization rules: it forgoes arithmetic decision procedures, but natively performs beta reduction and limited higher-order reasoning over predicate lambdas. The recent PLEX extension~\cite{ferrarini2026} adds normalization---eta-expansion, beta-reduction, and dependent pattern matching---to Liquid Haskell's Proof by Logical Evaluation layer, precisely to recover higher-order reasoning over a first-order SMT solver.

In the systems programming space, refinement types have been combined with mutation, where the central difficulty is deciding when established predicates must be invalidated: a refinement of a mutable location can be treated as an \emph{invariant} that every write must preserve, or as a flow-sensitive fact that a write updates or invalidates. Under aliasing, knowing which writes affect which facts is itself hard.
...
The accompanying artifact~\cite{bovel_2026_21737784} contains:

\begin{itemize}
  \item \textbf{Mechanization.} The Rocq formalization of the type system, with its soundness proof.
  \item \textbf{Implementation.} The source of our Dotty extension, also published on Maven and usable from any SBT project with:
\begin{lstlisting}[language=Scala]
scalaOrganization := "ch.epfl.lara"
scalaVersion := "3.10.0-RC1-bin-20260720-5f1bae1-NIGHTLY"
scalacOptions ++= Seq("-language:experimental.qualifiedTypes")
\end{lstlisting}
  \item \textbf{Benchmarks.} The JMH benchmark suites used for the evaluation, compiling similar programs with our Dotty fork, Schmid's prototype, and Stainless, together with scripts to run them and collect results.
\end{itemize}

The same material, with its continued development, is available in the project's public repository: \url{https://github.com/mbovel/first-class-refinement-types-in-scala}.
...
%% acmart prints "Received ...; accepted ..." from an \AtEndDocument hook,
%% which with an appendix lands it on a near-empty page after it. Print it
%% here, where the main matter ends, and empty the macro so the hook is a
%% no-op.
\makeatletter
