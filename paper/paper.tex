...
We present the design of first-class refinement types for Scala~3, where refinements are ordinary types that participate in subtyping, inference, and pattern matching alongside existing language features. We prove type soundness of a core, pure calculus mechanized in Rocq, combining dependent function types, bounded polymorphism, positive equi-recursive types, union and intersection types, and refinement types under a partial-correctness semantics using a fuel-bounded definitional interpreter and semantic typing. A distinctive design choice is that predicates themselves may diverge: soundness requires no termination checking, and we prove that a diverging predicate is vacuously true and can never be used to prove false. Finally, we implement our design as a prototype extension of the Scala~3 compiler with a lightweight e-graph-based solver for predicate entailment.
\end{abstract}
...
\begin{itemize}
  \item We present the \textbf{design} of first-class refinement types in Scala~3: their syntax, semantics, and interaction with Scala's existing type system, including subtyping, type inference, bounded polymorphism, overloading, and implicits (\S\ref{sec:design}).
  \item We give refinement predicates a \textbf{partial-correctness semantics that requires no termination checking} (\S\ref{sec:termination}). Predicates are ordinary, possibly diverging pure terms whose meaning is defined operationally rather than by translation into a classical logic: a refinement holds if its predicate evaluates to $\texttt{true}$ whenever it terminates. Type soundness requires no termination assumptions; termination resurfaces only when justifying individual entailment rules. We prove in Rocq that the design is not degenerate: a diverging predicate is indistinguishable from $\texttt{true}$ and can never be used to prove $\texttt{false}$ (Lemmas~\ref{lem:and-true}--\ref{lem:diverge-vacuous}).
